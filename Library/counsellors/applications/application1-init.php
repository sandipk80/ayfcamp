...
if(isset($_POST) && trim($_POST['add-application'])=="submit"){
    ...
    if(empty($error)){
        ...
        $applicationArray = array(
            ...
            'employer_indications' => $_POST['employer_indications'],
        ...
        if($saveApplication){
        	...
        	//store work history
        	if(is_array($_POST['employer_name']) && count($_POST['employer_name'])>0){
        		foreach($_POST['employer_name'] as $e=>$work){
        			$workArray = array(
        				'counsellor_id' => $counsellorId,
        				'application_id' => $applicationId,
        				'employer_name' => $work,
        				'dates' => $_POST['dates'][$e],
        				'supervisor' => $_POST['supervisor'][$e],
        				'employer_info' => $_POST['employer_info'][$e],
        				'work_nature' => $_POST['work_nature'][$e]
    				);
    				$saveHistory = $globalManager->runInsertQuery("counsellor_work_history", $workArray);
        		}
        	}
        	...
        	//store education
        	if(is_array($_POST['edu_year']) && count($_POST['edu_year'])>0){
        		foreach($_POST['edu_year'] as $e=>$edrow){
        			$eduArray = array(
        				'counsellor_id' => $counsellorId,
        				'application_id' => $applicationId,
        				'year' => $edrow,
        				'school_name' => $_POST['school_name'][$e],
        				'major' => $_POST['major'][$e],
        				'degree_granted' => $_POST['degree_granted'][$e]
    				);
    				$saveEdu = $globalManager->runInsertQuery("counsellor_educations", $eduArray);
        		}
        	}
            ...
            $_SESSION['message'] = "Thank you, your AYF Summer Camp Application has been submitted. An application submission does not ensure you a position in the AYF Summer Camp program. You will receive a phone call in the coming weeks for your interview. Please keep in mind that the Mandatory Counselor Orientation will be April 28-30, 2017 at AYF Camp.";
