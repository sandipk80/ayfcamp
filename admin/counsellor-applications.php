...
include(LIB_PATH . 'admin/counsellors/applications-init.php');
                                    ...
                                    <tbody>
                                        ...
                                        if(is_array($result) && count($result)>0){
                                            foreach($result as $row){
                                                $deleteItem=ADMIN_SITE_URL."counsellor-applications.php?act=delete&id=".$row['id'];
                                                $address = "";
                                                if($row['address'] !== ""){
                                                    $address .= $row['address'];
                                                }
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a class="btn btn-success" href="<?php echo ADMIN_SITE_URL.'counsellor-application.php?id='.$row['id'];?>">View</a>
...
$(document).on("click", ".updateStatus", function (e) {
    ...
    if(confirm("Are you sure want to update the status of this counsellor?")) {
        $.ajax({
            ...
            dataType: "json",
            method: "post",
            data: {id:id,st:status,tb:'counsellor_applications'},
            cache: false,
